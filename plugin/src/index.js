    ...
    config = withAppDelegate(config, (delegateConfig) => {
        ...
        if (delegateConfig.modResults.language === 'swift') {
            ...
            if (!delegateConfig.modResults.contents.includes('Terra.setUpBackgroundDelivery()')) {
                ...
                delegateConfig.modResults.contents =
                    delegateConfig.modResults.contents.replace(regex, (match) => `Terra.setUpBackgroundDelivery()\n  ${match}`);
            }
        }
        else {
            ...
            if (!contents.includes('[Terra setUpBackgroundDelivery];')) {
                ...
                delegateConfig.modResults.contents =
                    delegateConfig.modResults.contents.replace(regex, (match) => `${match}  [Terra setUpBackgroundDelivery];\n`);
            }
        }
...
export default createRunOncePlugin(withTerraBackgroundDelivery, pkg.name, pkg.version);
