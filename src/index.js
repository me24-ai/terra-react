...
export function initConnection(connection, token, schedulerOn, customPermissions_ = [], startIntent = null, customWritePermissions_ = []) {
    return TerraReact.initConnection(ConnectionToString(connection), token, schedulerOn, customPermissions_.map((p) => CustomPermissions_ToString(p)), startIntent, customWritePermissions_.map((p) => CustomPermissions_ToString(p)));
}
...
export function getBody(connection, startDate, endDate, latestReading = false, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getBody(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), latestReading, toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
...
export function getActivity(connection, startDate, endDate, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getActivity(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
                data: d.data !== undefined ? JSON.parse(d.data) : null,
...
export function getMenstruation(connection, startDate, endDate, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getMenstruation(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
                data: d.data !== undefined ? JSON.parse(d.data) : null,
...
export function getDaily(connection, startDate, endDate, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getDaily(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
                data: d.data !== undefined ? JSON.parse(d.data) : null,
...
export function getNutrition(connection, startDate, endDate, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getNutrition(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
                data: d.data !== undefined ? JSON.parse(d.data) : null,
...
export function getSleep(connection, startDate, endDate, toWebhook = true) {
    return new Promise((resolve, reject) => {
        TerraReact.getSleep(ConnectionToString(connection), startDate.toISOString(), endDate.toISOString(), toWebhook)
            .then((d) => {
            const data = {
                success: d.success,
                data: d.data !== undefined ? JSON.parse(d.data) : null,
...
export function postActivity(connection, payload) {
    ...
    const timeout = new Promise((resolve) => setTimeout(() => resolve({ success: false, error: 'postActivity timed out' }), POST_TIMEOUT_MS));
    return Promise.race([native, timeout]);
}
/*
@Only available on iOS
*/
export function postNutrition(connection, payload) {
    if (Platform.OS !== 'ios') {
    ...
    const timeout = new Promise((resolve) => setTimeout(() => resolve({ success: false, error: 'postNutrition timed out' }), POST_TIMEOUT_MS));
    return Promise.race([native, timeout]);
}
/*
@Only available on iOS
*/
export function postBody(connection, payload) {
    if (Platform.OS !== 'ios') {
        return Promise.resolve({
            success: false,
    ...
    const timeout = new Promise((resolve) => setTimeout(() => resolve({ success: false, error: 'postBody timed out' }), POST_TIMEOUT_MS));
    return Promise.race([native, timeout]);
}
export function getPlannedWorkouts(connection) {
    return new Promise((resolve, reject) => {
...
export function deletePlannedWorkout(connection, id) {
    return new Promise((resolve, reject) => {
...
export function completePlannedWorkout(connection, id, at) {
    return new Promise((resolve, reject) => {
        TerraReact.completePlannedWorkout(ConnectionToString(connection), id, (at ?? new Date()).toISOString())
            .then((d) => {
            const res = {
                success: d.success,
                error: d.error,
...
export function postPlannedWorkout(connection, payload) {
    return new Promise((resolve, reject) => {
        TerraReact.postPlannedWorkout(ConnectionToString(connection), JSON.stringify(payload.toJson()))
            .then((d) => {
            const res = {
                success: d.success,
                error: d.error,
 ...
 */
export function getAllHealthKitPermissions() {
    if (Platform.OS !== 'ios') {
 ...
 */
export function requestAllHealthKitPermissions() {
    if (Platform.OS !== 'ios') {
        return Promise.resolve({ success: false, error: 'Only available on iOS' });
    }
